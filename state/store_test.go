...
import (
	...
	ss "github.com/scalarorg/abci/state"
)
...
func TestStoreLoadValidators(t *testing.T) {
	...
	err := ss.SaveValidatorsInfo(stateDB, 1, 1, vals)
	require.NoError(t, err)
	err = ss.SaveValidatorsInfo(stateDB, 2, 1, vals)
	require.NoError(t, err)
	...
	err = ss.SaveValidatorsInfo(stateDB, ss.ValSetCheckpointInterval, 1, vals)
	require.NoError(t, err)

	loadedVals, err = stateStore.LoadValidators(ss.ValSetCheckpointInterval)
	require.NoError(t, err)
	assert.NotZero(t, loadedVals.Size())
}
...
func BenchmarkLoadValidators(b *testing.B) {
	...
	stateStore := ss.NewStore(stateDB, ss.StoreOptions{
		DiscardABCIResponses: false,
	})
	...
	for i := 10; i < 10000000000; i *= 10 { // 10, 100, 1000, ...
		i := i
		if err := ss.SaveValidatorsInfo(stateDB,
...
func TestPruneStates(t *testing.T) {
	...
	for name, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			db := dbm.NewMemDB()
			stateStore := ss.NewStore(db, ss.StoreOptions{
				DiscardABCIResponses: false,
			})
			...
			for h := int64(1); h <= tc.makeHeights; h++ {
				...
				err = stateStore.SaveABCIResponses(h, &tstate.ABCIResponses{
			...
			for h := int64(1); h <= tc.makeHeights; h++ {
				...
				if expectVals[h] {
					...
					require.Equal(t, cmts.ErrNoValSetForHeight{Height: h}, err)
				}
				...
				if expectABCI[h] {
					...
					require.Equal(t, cmts.ErrNoABCIResponsesForHeight{Height: h}, err)
				}
			}
		})
	}
...
func TestABCIResponsesResultsHash(t *testing.T) {
	responses := &tstate.ABCIResponses{
	...
	root := cmts.ABCIResponsesResultsHash(responses)
...
func TestLastABCIResponses(t *testing.T) {
	...
	t.Run("Not persisting responses", func(t *testing.T) {
		...
		response1 := &tstate.ABCIResponses{
			BeginBlock: &abci.ResponseBeginBlock{},
			DeliverTxs: []*abci.ResponseDeliverTx{
				{Code: 32, Data: []byte("Hello"), Log: "Huh?"},
			},
		...
		stateDB = dbm.NewMemDB()
		stateStore = cmts.NewStore(stateDB, cmts.StoreOptions{DiscardABCIResponses: false})
		height := int64(10)
	...
	t.Run("persisting responses", func(t *testing.T) {
		...
		response2 := &tstate.ABCIResponses{
			BeginBlock: &abci.ResponseBeginBlock{},
			DeliverTxs: []*abci.ResponseDeliverTx{
		...
		assert.Equal(t, cmts.ErrABCIResponsesNotPersisted, err)
	})
}
func TestIntConversion(t *testing.T) {
	x := int64(10)
	b := ss.Int64ToBytes(x)
	require.Equal(t, x, ss.Int64FromBytes(b))
}
