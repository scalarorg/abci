...
var _ cmtstate.Store = (*dbStore)(nil)
...
func NewStore(db dbm.DB, options StoreOptions) cmtstate.Store {
	return dbStore{db, options}
}

// LoadStateFromDBOrGenesisFile loads the most recent state from the database,
// or creates a new one from the given genesisFilePath.
func (store dbStore) LoadFromDBOrGenesisFile(genesisFilePath string) (cmtstate.State, error) {
	state, err := store.Load()
	if err != nil {
	...
	if state.IsEmpty() {
		var err error
		state, err = cmtstate.MakeGenesisStateFromFile(genesisFilePath)
		if err != nil {
			return state, err
		}
	}
...
func (store dbStore) LoadFromDBOrGenesisDoc(genesisDoc *types.GenesisDoc) (cmtstate.State, error) {
	state, err := store.Load()
	if err != nil {
		return cmtstate.State{}, err
	}
	...
	if state.IsEmpty() {
		var err error
		state, err = cmtstate.MakeGenesisState(genesisDoc)
		if err != nil {
			return state, err
		}
	}
...
func (store dbStore) Load() (cmtstate.State, error) {
	return store.loadState(stateKey)
}

func (store dbStore) loadState(key []byte) (state cmtstate.State, err error) {
...
func (store dbStore) Save(state cmtstate.State) error {
	return store.save(state, stateKey)
}

func (store dbStore) save(state cmtstate.State, key []byte) error {
...
func (store dbStore) Bootstrap(state cmtstate.State) error {
...
func ABCIResponsesResultsHash(ar *tstate.ABCIResponses) []byte {
...
func (store dbStore) LoadABCIResponses(height int64) (*tstate.ABCIResponses, error) {
	if store.DiscardABCIResponses {
		return nil, cmtstate.ErrABCIResponsesNotPersisted
	}
	...
	if len(buf) == 0 {
		return nil, cmtstate.ErrNoABCIResponsesForHeight{height}
	}

	abciResponses := new(tstate.ABCIResponses)
...
func (store dbStore) LoadLastABCIResponse(height int64) (*tstate.ABCIResponses, error) {
	...
	abciResponse := new(tstate.ABCIResponsesInfo)
...
func (store dbStore) SaveABCIResponses(height int64, abciResponses *tstate.ABCIResponses) error {
	...
	response := &tstate.ABCIResponsesInfo{
...
func (store dbStore) LoadValidators(height int64) (*types.ValidatorSet, error) {
	...
	if err != nil {
		return nil, cmtstate.ErrNoValSetForHeight{height}
	}
	if valInfo.ValidatorSet == nil {
...
func loadValidatorsInfo(db dbm.DB, height int64) (*tstate.ValidatorsInfo, error) {
...
func (store dbStore) loadConsensusParamsInfo(height int64) (*tstate.ConsensusParamsInfo, error) {
	...
	paramsInfo := new(tstate.ConsensusParamsInfo)
...
func (store dbStore) saveConsensusParamsInfo(nextHeight, changeHeight int64, params types.ConsensusParams) error {
	paramsInfo := &tstate.ConsensusParamsInfo{
