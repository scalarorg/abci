...
func (s *syncer) SyncAny(discoveryTime time.Duration, retryHook func()) (cmts.State, *types.Commit, error) {
	...
	for {
		...
		if snapshot == nil {
			if discoveryTime == 0 {
				return cmts.State{}, nil, errNoSnapshots
			}
			retryHook()
			s.logger.Info("sync any", "msg", log.NewLazySprintf("Discovering snapshots for %v", discoveryTime))
			time.Sleep(discoveryTime)
		...
		if chunks == nil {
			...
			if err != nil {
				return cmts.State{}, nil, fmt.Errorf("failed to create chunk queue: %w", err)
			}
		...
		default:
			return cmts.State{}, nil, fmt.Errorf("snapshot restoration failed: %w", err)
		}
...
func (s *syncer) Sync(snapshot *snapshot, chunks *chunkQueue) (cmts.State, *types.Commit, error) {
	s.mtx.Lock()
	if s.chunks != nil {
		s.mtx.Unlock()
		return cmts.State{}, nil, errors.New("a state sync is already in progress")
	}
	s.chunks = chunks
	s.mtx.Unlock()
	defer func() {
	...
	if err != nil {
		...
		return cmts.State{}, nil, errRejectSnapshot
	}
